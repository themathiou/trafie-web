...
exports.post = function( req, res ){
	if( typeof req.body.feedback_text !== 'undefined' ) {
		...
		if( typeof req.session.user_id !== 'undefined' ) {
			User.schema.findOne({ '_id': req.session.user_id }, 'email').then( function( user ) {
				user_data_text = '<b>Email</b>: ' + user.email + '<br>';
				return Profile.schema.findOne({ '_id': req.session.user_id }, '_id first_name last_name discipline country language birthday');
			})

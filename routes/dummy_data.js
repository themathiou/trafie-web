...
var password = User.schema.encryptPassword('123123');
...
function create_user( max_users, res ) {
	if( counter < max_users ) {
	    ...
	    var new_user = {
	      'email': first_name.toLowerCase() + delimiter + last_name.toLowerCase() + (~~( Math.random() * 1000 ) ) +'@trafie.com',
	      'password': password,
	      'valid': true
	    };
	    ...
	    var new_profile = {
	      ...
	      'last_name': last_name,
	      'username': first_name.toLowerCase() + delimiter + last_name.toLowerCase() + (~~( Math.random() * 1000 ) )
	    };
